...
namespace CryptoExchanges.Net.Bybit.Tests.Integration.Streaming;

/// <summary>
/// Live integration smoke tests for the Bybit v5 public WebSocket streaming client.
/// Self-skip when <c>wss://stream.bybit.com/v5/public/spot</c> is unreachable (offline → skip,
...
public class BybitStreamingSmokeTests
{
    ...
    // reconnect loop on Bybit v5 (100 ms MinOutboundInterval, batch builder).
    ...
    /// <summary>
    /// Verifies the Bybit v5 public spot WebSocket gateway is reachable. Returns <c>null</c>
    ...
    private static async Task<string?> CheckReachabilityAsync()
    {
        ...
        {
            return "Bybit WebSocket endpoint unreachable (connectivity) — skipping integration streaming smoke tests.";
        }
    }

    /// <summary>
    /// Builds a scoped DI service provider that wires both <c>AddBybitExchange</c> and
    /// <c>AddBybitStreams</c> with empty credentials (public streams require none).
    /// </summary>
    private static IStreamClient BuildStreamClient()
    {
        var services = new ServiceCollection();
        services.AddBybitExchange(o =>
        {
        ...
        return factory.GetClient(ExchangeId.Bybit);
    }
    ...
    /// <summary>
    /// Regression gate for the FEAT-008 multi-symbol reconnect-loop bug. Subscribes to ≥8 Bybit
