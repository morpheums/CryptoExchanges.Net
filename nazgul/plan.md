...
- **Current Task:** **TASK-018 IMPLEMENTED** (BitgetSignatureService base64 HMAC via Core HmacSignature + BuildPrehash incl. `?`+query when non-empty + epoch-ms FormatTimestamp + BitgetSigningRequest marker `bitget.signed`). ISignatureService impl, internal, lean comments. Build 0W/0E; commit d268e2a; diff at nazgul/reviews/TASK-018/diff.patch. Awaiting review gate. TASK-017 prior IMPLEMENTED (scaffold, commit 9029eab). 19/25 done; TASK-018 implemented.
- **Next Action:** Review gate for TASK-018. Then Wave 13 = **TASK-020** (BitgetSymbolFormat delimiter-less `BTCUSDT`) → 019/021 (signing handler `ACCESS-KEY/SIGN/TIMESTAMP/PASSPHRASE` + http client) → 022 closer (services+mapping+composer+client+error+time+tests+AddBitgetExchange).
