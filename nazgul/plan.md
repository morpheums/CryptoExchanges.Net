...
- **OPEN DESIGN QUESTION (user, 2026-06-18):** maintainer questioned the DI design in ServiceCollectionExtensions.cs (DI package hard-references every exchange assembly; AddCryptoExchanges registers all unconditionally; ~90% duplication per exchange). architect-reviewer dispatched for options/recommendation — may reshape how OKX/Bitget DI is done (possibly fold a DI refactor into TASK-009). AWAIT architect + user decision before OKX DI work.
