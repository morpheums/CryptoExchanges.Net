...
public class CoinbaseRestSmokeTests : IAsyncLifetime
{
    private static readonly Symbol BtcUsd = new(Asset.Btc, Asset.Of("USD"));

    private CoinbaseExchangeClient _client = null!;
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        {
            _skipReason = "Coinbase REST endpoint unreachable (timeout) — skipping integration smoke tests.";
        }
    }
    ...
    {
        SkipIfUnavailable();
        Assert.SkipWhen(!_hasCredentials, "COINBASE_API_KEY not set — skipping credential-required smoke test.");
    }
