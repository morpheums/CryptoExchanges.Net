...
7. **Lean comments / docs on interfaces (maintainer mandate, 2026-06-18)** — XML docs live on the **interface**; implementations use `/// <inheritdoc />` (never repeat the doc block). One concise `<summary>` per interface member; add `<param>`/`<returns>`/`<exception>` only when they add information the signature doesn't. NO `<remarks>` essays, NO comments restating the code, NO inline comment on every line — comment non-obvious *why*, not *what*. Over-commenting is a reviewable defect.
8. **Interfaces over static classes for swappable behavior (DIP, 2026-06-18)** — behaviors the maintainer might replace (time-sync, signing, mapping, translation) are interfaces resolved via DI, not `static` classes. `static` only for fixed pure helpers + DI extension glue. When unsure, prefer the interface.
